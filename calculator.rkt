...
(define (read-digits cs)
  (let loop ([c cs] [acc '()])
    (cond
      [(and (pair? c) (char-numeric? (car c))) (loop (cdr c) (cons (car c) acc))]
      [else (if (null? acc) #f (values (string->number (list->string (reverse acc))) c))])))
...
;; BUG: uses `/` (floating) and no zero check
    ...
    [else (define c (car cs*))
          (cond
            ...
            [(char=? c #\$)
             ...
             (call-with-values (λ() r) (λ(n rest) (values (hist-get hist n) rest)))]
            [(char-numeric? c)
             (define r (read-int cs*))
             (if r (call-with-values (λ() r) (λ(n rest) (values n rest)))
...
(define (repl hist)
  ...
  (cond
    ...
    [else
     (with-handlers ([exn:fail? (λ(_)(displayln "Error: Invalid Expression")(repl hist))])
